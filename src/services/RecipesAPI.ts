...
const fetchBase = async (URL: string): Promise<APIDataType> => {
  let data;
  try {
...
export const fetchAllRecipes = async (type: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/search.php?s=`));

export const fetchRecipeById = async (type: string, id: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/lookup.php?i=${id}`));

export const fetchRecipesByName = async (type: string, name: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/search.php?s=${name}`));

export const fetchRecipesByFirstLetter = async (type: string, letter: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/search.php?f=${letter}`));

export const fetchRecipesByIngredient = async (type: string, ingredient: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/filter.php?i=${ingredient}`));

export const fetchRecipesByCategory = async (type: string, category: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/filter.php?c=${category}`));

export const fetchCategories = async (type: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/list.php?c=list`));

export const fetchIngredients = async (type: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/list.php?i=list`));

export const fetchRandomRecipe = async (type: string): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(type)}/random.php`));

export const fetchAllMealsAreas = async (): Promise<APIDataType> => (
  fetchBase(`${getBaseURL(MEALS_TYPE)}/list.php?a=list`));

export const fetchMealsByArea = async (area: string): Promise<APIDataType> => (
